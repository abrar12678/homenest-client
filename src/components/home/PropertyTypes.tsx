  ...
  useEffect(() => {
    getStats()
      .then((res) => {
        ...
        if (data?.propertiesByType) {
          const counts: Record<string, number> = {};
          for (const item of data.propertiesByType) {
            ...
            return (
              <motion.div key={item.type} variants={cardVariants}>
                <Link href={`/properties?type=${item.type}`} className="block h-full">
                  <div className="group relative bg-white rounded-2xl border border-slate-100 p-6 text-center h-full overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2.5 hover:border-slate-200 cursor-pointer hover:scale-[1.02]">
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                      {/* Icon */}
                      <div
                        className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${item.gradient} flex items-center justify-center mb-5 shadow-lg group-hover:bg-white/20 group-hover:scale-[1.15] transition-all duration-300 group-hover:rotate-3`}
                      >
